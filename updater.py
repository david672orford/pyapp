...
class PackageUpdater:
	...
	def install(self):
		...
		# The update_dir may be empty now, so try to remove it.
		try:
		...
		if os.path.exists(updater):
			...
			try:
				...
				exec(updater_code, globals(), {'package_dir':self.package_dir, 'saved_cwd':saved_cwd})
...
def download_simple_update(show_status):
	# Path to code to be updated
	app_dir = sys.path[0]

	# Create a temporary directory at the same level.
